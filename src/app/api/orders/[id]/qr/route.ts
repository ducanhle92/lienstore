...
export async function GET(_req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  const cfg = await accountForOrder(order);
  const png = await QRCode.toBuffer(orderQrPayload(cfg, order.total, order.payCode), { type: "png", width: 512, margin: 2, errorCorrectionLevel: "M" });
