...
export async function GET(req: NextRequest) {
  ...
  const lines = rows.map((r) => [`#${r.number}`, r.createdAt.slice(0, 10), r.customer, r.status, r.paid ? "x" : "", r.items, r.revenue, r.shipOnDelivery ? "trả shipper" : r.shipCollected, r.cogs, r.importFees, r.vnCarrierFee, r.profit, r.missingCost, r.voucher, r.promoDiscount]);
  lines.push(["Tổng", `${from} → ${to}`, "", "", totals.paidOrders, "", totals.revenue, totals.shipCollected, totals.cogs, totals.importFees, totals.vnCarrierFee, totals.profit, totals.missingCost, totals.voucher, totals.promoDiscount]);
