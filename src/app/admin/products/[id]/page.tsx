...
import { getCategories, getImportQuoteConfig, getMonthlyUnitsSold, getPricingConfig, getProductById, getPurchaseSourceDefault, listCostSources, listProductGroups, listPurchaseSources } from "@/lib/db";
...
export default async function EditProduct({ params }: Props) {
  ...
  return (
    <>
      ...
      />
      <ProductForm product={product} categories={categories} quote={quote} pricing={pricing} skuSuggestion={skuSuggestion} costSources={costSources} defaultSource={defaultSource} groups={groups} sources={sources} monthlySales={getMonthlyUnitsSold(product.id, 6)} />
    </>
  );
}
