...
export async function SectionHeader2({ title, href, icon, tone = "blue", className }: { title: string; href?: string; icon?: FaName; tone?: "blue" | "sale"; className?: string }) {
...
const USP: Array<{ icon: FaName; title: "usp1t" | "usp2t" | "usp3t" | "usp4t"; text: "usp1x" | "usp2x" | "usp3x" | "usp4x" }> = [
  { icon: "check-circle", title: "usp1t", text: "usp1x" },
  { icon: "plane", title: "usp2t", text: "usp2x" },
  { icon: "comments-o", title: "usp3t", text: "usp3x" },
  { icon: "shield", title: "usp4t", text: "usp4x" },
];
    ...
    <section aria-label="Cam kết" className="my-10 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
      {USP.map((u) => (
        <div key={u.title} className="flex items-start gap-3 rounded-md border border-lien-line bg-white p-4">
          ...
          <div>
            <p className="m-0 text-[14px] font-bold leading-5 text-lien-heading">{t(lang, u.title)}</p>
            <p className="m-0 mt-0.5 text-[13px] leading-5 text-lien-muted">{t(lang, u.text)}</p>
          </div>
        </div>
      ))}
    </section>
...
export async function NewsCards({ posts }: { posts: BlogPost[] }) {
  if (posts.length === 0) return null;
  const lang = await getLang();
  return (
    <section aria-label="Tin tức" className="my-10">
      <SectionHeader2 title={t(lang, "newsTitle")} href="/category/goc-chia-se/" icon="newspaper-o" />
        ...
        {posts.slice(0, 4).map((p) => (
          <li key={p.slug} className="flex flex-col rounded-md border border-lien-line bg-white p-4 hover:shadow-[0_8px_24px_-12px_rgba(0,0,0,0.3)]">
            ...
            <Link href={`/${p.slug}/`} className="mt-auto pt-3 text-[13px] font-medium text-lien-blue no-underline hover:underline">
              {t(lang, "readMore")} <Fa name="angle-right" />
            </Link>
          </li>
        ))}
      </ul>
...
export async function PageBand({ title, crumbs, description }: { title: string; crumbs: Array<{ label: string; href?: string }>; description?: string }) {
  const lang = await getLang();
  return (
