...
- **Thanh toán**: chọn nhận tại kho (miễn phí) hoặc giao tận nhà theo khu vực (phí từ bảng vận chuyển nội địa, miễn phí khi đạt mức), COD hoặc chuyển khoản; đơn có hàng order bắt buộc chuyển khoản 100%; trang "đã nhận đơn" hiện tài khoản BIDV + mã VietQR điền sẵn số tiền và nội dung `LIENSTORE <mã đơn>`; giá tính lại phía server; tra cứu đơn bằng mã đơn + số điện thoại.
...
- **Phí vận chuyển theo đơn**: cân tính phí = max(cân thật, D×R×C/6000) × hệ số an toàn theo độ tin cậy kích thước (Cao ×1,2 · TB ×1,5 · Thấp ×2); trang thanh toán cộng 3 chặng (nội địa Nhật, Nhật → VN, nội địa VN; tự tới kho thì bỏ chặng VN) khi bật chế độ "Tính riêng 3 chặng" (Vận chuyển › Hiển thị cho khách, kèm tỷ giá ¥).
- **Đơn hàng**: tiến độ vận chuyển 7 bước (admin bấm chuyển bước, khách xem thanh tiến độ), chat khách ↔ shop theo đơn (huy hiệu chưa đọc ở danh sách), 3 chặng vận chuyển, bill mua hàng tại Nhật.
- **Sales** — Khách hàng; **Giảm giá sản phẩm** (`/admin/promotions/discounts/`: đặt giá KM theo giá hoặc %, bỏ giảm); **Voucher** (`/admin/promotions/vouchers/`: mã giảm %/số tiền, đơn tối thiểu, giảm tối đa, thời hạn, số lượt; khách nhập ở trang thanh toán). Quyền `promotions`.
- **Tiếng Nhật**: sản phẩm/danh mục có trường tiếng Nhật (admin, Excel); `fetch_ja_content.py` cào nội dung gốc từ Amazon.co.jp; giao diện 日本語 trên mọi trang.
- **Storefront**: chuyển ngôn ngữ khung VI | JP ở thanh trên (cookie `lien_lang`); icon người ở header mở ngăn kéo đăng nhập / đăng ký (ID bắt buộc, email tuỳ chọn) hoặc menu tài khoản.
- **Vận chuyển** (`/admin/shipping/`): 3 chặng (nội địa Nhật, Nhật → Việt, nội địa Việt Nam); mỗi phương thức có đơn vị vận chuyển, giá gồm 2 đầu hay chưa, kho/địa điểm, giao tận nhà, lưu ý; cột bảng là khu vực hoặc bậc cân nặng (phí, đơn vị /kg, miễn phí trên, phụ phí, thời gian); quản lý đơn vị vận chuyển; xem trước như khách thấy. Tab **Đơn hàng**: mỗi đơn một dòng, 3 chặng theo chiều ngang, chọn phương thức · cột, phí (tự tính theo cân của đơn nếu để trống), mã vận đơn, ghi chú; chặng nội địa VN áp lại được phí vào tổng khách trả. Sản phẩm có khối lượng/kích thước thì trang sản phẩm ước tính phí gửi và trang thanh toán tính phí theo cân toàn đơn (`scripts/xlsx/fetch_dimensions.py` cào dữ liệu này).
